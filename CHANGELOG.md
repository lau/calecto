...
- CalectoDateTimeUTC.dump is now ~40% faster according to certain benchmarks.

## Changed

- Ecto compatability changed to {:ecto, "~> 1.1.3 or ~> 2.0.0 or ~> 2.0.0-rc3"}.
  If using 2.0.x avoid 2.0.0-rc4. Use rc3 or rc5 (if/when available).

- Compatability with Calendar changed to version 0.14.1.

### Fixed

- Ecto 2.0.0-rc4 had a change that made autogenerated timestamps
  incompatible with the use macro in Calecto.Schema. This release
  has a fix and should work with the next Ecto release candidate.

### Removed

- Unused structs e.g. %Calecto.NaiveDateTime{} %Calecto.DateTime{} etc.
  They were deprecated in 0.5.0. Now they are removed.
